...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ENCPMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ENCPMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ENCPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("enceladus-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ENCPMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ENCPMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadENCPMiner() exception");
    } catch (...) {
        LogPrintf("ThreadENCPMiner() exception");
    }

    LogPrintf("ThreadENCPMiner exiting\n");
}
