...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ENCP address (e.g. %1)").arg("EFcDRgUinXTYpFFE18n83poxt4Q4Su5T5W"));
#endif
...
{
    // return if URI is not valid or is no ENCP: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::ENCP, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert enceladus:// to enceladus:
    //
    //    Cannot handle this later, because enceladus:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ENCP, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open enceladus.conf with the associated application */
                                                                  ...
                                                                  value(amount)
{
    ...
    if (amount > 0)
    {
        res = BitcoinUnits::floorWithUnit(BitcoinUnits::ENCP, amount);
    }
    else
    {
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ENCP.lnk";
}
...
{
    return GetAutostartDir() / "enceladus.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a enceladus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ENCP\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the enceladus app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add enceladus app to startup item list
